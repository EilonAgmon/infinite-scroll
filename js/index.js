...
$(document).ready(function() {
	...
	win.scroll(function() {
		...
		if (win.scrollTop() >= $(document).height() - win.height() - 25) {
			doMyShit();
		}
	});
});
...
function init() {
	...
	for (i = 0; i < 50; i++) { 
			var post = '<li>';
			post += '<article>';
			post += '<header><h1>'+ Math.random() + '</h1></header>';
			post += '</article>';
			post += '</li>';
			$('#posts').append(post);
		}
...
function doMyShit() {
    ...
    success: function(response) {
		...
		for (i = 0; i < response.messages.length; i++) { 
			var post = '<li>';
			post += '<article>';
			post += '<header><h1>'+response.messages[i].author.name + Math.random() + '</h1></header>';
			post += '</article>';
			post += '</li>';
			$('#posts').append(post);
		}
